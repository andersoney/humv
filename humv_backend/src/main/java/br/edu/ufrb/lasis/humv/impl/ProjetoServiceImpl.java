...
public Projeto findById(String id){
	return projetoDAO.findByKey(id);
}
...
public String cadastrarProjeto(Projeto projeto, String usuarioResponsavel){
	try{
		...
		if(ex.getMessage().toLowerCase().contains("constraint")){
			...
			return "Projeto com Nome " + projeto.getNome() + " ja cadastrado no sistema. Por favor, informe um Nome diferente.";
		}else{
...
public String removerProjeto(String id, String usuarioResponsavel){
	if(projetoDAO.findByKey(id)==null){
	...
	Projeto projeto = projetoDAO.findByKey(id);
